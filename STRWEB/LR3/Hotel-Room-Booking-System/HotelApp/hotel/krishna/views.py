...
def contact_page(request):
    if request.method == 'GET':
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            contacts_list = list(contacts.values('id', 'name', 'description', 'phonenumber', 'email', 'img'))
            return JsonResponse(contacts_list, safe=False)
        return render(request, 'contact.html', {'contacts': contacts})

@require_POST
def update_contact_description(request):
...
def add_contact(request):
    try:
        ...
        print(f"Received data: name={name}, description={description}, phonenumber={phonenumber}, email={email}, img={img}")

        contact = Contacts(name=name, description=description, phonenumber=phonenumber, email=email, img=img)
        contact.save()

        return JsonResponse({'status': 'success', 'contact': {
            'id': contact.id,
            'name': contact.name,
